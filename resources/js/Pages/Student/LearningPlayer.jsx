                    ...
                    <div className="xl:col-span-2 rounded-2xl border border-border bg-card p-5 shadow-card space-y-4">
                        ...
                        {activeLesson?.content_type === 'video' && activeLesson?.video_url && (
                            <div className="aspect-video rounded-2xl overflow-hidden border border-border bg-black">
                                <iframe className="w-full h-full" src={activeLesson.video_url} title={activeLesson.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
                            </div>
                        )}
                                ...
                                <div key={module.id} className="rounded-xl border border-border bg-background p-4 space-y-3">
                                    ...
                                    <div className="space-y-2">
                                        {module.lessons.map((lesson) => (
                                            <div key={lesson.id} className="flex items-start justify-between gap-3 text-sm">
                                                <div>
